...
@main struct ModelChecks {
    static func main() throws {
        for invalid in ["", "-oProxyCommand=evil", "user@host;id", "host\ncommand", "$(id)", "a b"] {
            precondition(!validSSHAddress(invalid), invalid)
        }
        ...
        let spec = RuntimeClient.terminalSpec(
            host: Host(name: "Remote", address: "test", managed: false),
            run: Run(
                id: "x", taskId: "x", workspaceId: "w", title: "SSH", agent: "shell", prompt: "", cwd: "/tmp/a'b",
                session: "", status: "direct", createdAt: 0), health: nil)
        ...
        let original = Workspace(
            id: "w", name: "离线修改", path: "/tmp", hostID: "remote", split: true, archived: true, syncPending: true)
        var server = ServerWorkspace(original)
        server.name = "旧的远端名称"
        ...
        precondition(layout.paneRunIDs == ["d"], "An independent tab replaces the whole page")
        layout.selectTerminal("a", available: available)
        precondition(layout.paneRunIDs == ["a", "b", "c"], "Returning to the group restores its panes")
        ...
        precondition(
            layout.visibleRunIDs(available: ["a", "c"]) == ["a", "c"],
            "Archived sessions should disappear without displacing surviving panes")
        ...
        let serverLayout = try JSONDecoder().decode(
            ServerWorkspace.self, from: JSONEncoder().encode(ServerWorkspace(layout)))
        precondition(serverLayout.workspace(hostID: "remote").paneRunIDs == ["a", "b", "c"])
        let stale = Workspace(id: "stale", name: "Stale", path: "/tmp", paneRunIDs: ["a", "a", "missing", "c"])
        precondition(stale.visibleRunIDs(available: available) == ["a", "c"])
        layout.setPanes(["c"], focused: "a")
        precondition(!layout.split && layout.selectedRunID == "c" && layout.secondaryRunID == nil)
        print(
            "PASS dynamic split: left/right insertion, movement, focus, tab replacement, invalid drops, cleanup, legacy migration and persistence"
        )
