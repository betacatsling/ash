...
CORE=(Sources/Ash/Models.swift Sources/Ash/KeyboardShortcuts.swift Sources/Ash/HostConfiguration.swift Sources/Ash/RuntimeClient.swift Sources/Ash/TerminalEnvironment.swift)
PERSISTENCE=(Sources/Ash/Persistence.swift Sources/Ash/WorkspaceChecklistStore.swift)
STORE=("${CORE[@]}" "${PERSISTENCE[@]}" Sources/Ash/AppStore.swift Sources/Ash/AppStore+Hosts.swift Sources/Ash/HostConnectionService.swift Sources/Ash/RemoteRuntimeManager.swift Sources/Ash/SSHConfigDiscovery.swift)
PANEL=("${CORE[@]}" Sources/Ash/SidePanelModel.swift Sources/Ash/PanelBrowserModel.swift)
case "$CHECK" in
  ModelChecks|TerminalEnvironmentChecks) SOURCES=("${CORE[@]}") ;;
