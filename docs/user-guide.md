...
在「设置 → 终端快捷键」统一配置「按编号切换终端」和左、右切换，共三项。编号切换只需录入修饰键加任意数字 1–9，即可同时更新整组数字键；默认 ⌘1–⌘9。Esc 取消录入；修改立即保存，支持冲突提示和恢复默认。数字键按标签页从左到右选择，组合页计为一个标签；切换独立页会占满整个终端区域，返回组合页则恢复原分屏。

双击独立终端标签，或右键选择「重命名…」，可以设置名称；组合页从成员子菜单分别重命名。支持中文和恢复原名称。名称保存在当前 Mac，后台刷新和重启后保留，本地与 SSH 终端均可使用。

选中的标签页显示关闭叉号，也可右键选择「关闭标签页」。组合页的成员子菜单可只关闭某一个终端，其余面板保留。关闭会停止会话并归档记录，保留工作区与文件；停止失败时保留标签并提示错误。

按 ⌘W 或标签上的叉号关闭当前标签页；组合页会依次停止并归档其中所有会话。可通过成员子菜单只关闭一个终端。若停止失败，保留失败的会话和未处理成员。关闭最后一个标签页后返回工作区欢迎页，应用窗口保持打开。

终端内容直接接在标签栏下方。按 ⌘F 在当前终端右上角打开查找浮层，输入后高亮匹配，Enter 或上下箭头切换结果；Esc 关闭并回到终端输入。浮层不改变终端尺寸。终端输出持续更新，托管连接中断会自动重连，无需手动刷新。

拖动顶部标签时会出现浮动标签和落点提示。拖到另一个标签上合并为一页，默认左右排列，按住 ⌥ 松开改为上下排列；拖到终端的左、右、上、下边缘可选择具体落点。合并时标签收拢并高亮，组合页显示成员名称与数量。Esc 或拖到窗口外松开可取消。接缝只显示 1pt 细线，附近 9pt 范围可拖动调整大小。

每个组合页保留自己的分屏、比例和焦点。点击其他标签会切换整个页面；例如 A/B 合并后，点击独立的 C 会全页显示 C，点击 A/B 则恢复两者。右键组合页可选择「左右排列」「上下排列」「拆为独立标签页」，也可只将某个成员移到独立页。⌘D 可快速合并另一个标签或拆开当前组。各组完整布局保存在本机，重启后恢复；旧版可见分屏自动迁移为一个组合页。同一个会话只属于一个标签页，合并、拆开和切换不会重新启动进程。

组合页中的终端名称可以分别点击和拖动。把某个名称拖到标签栏空白处，就会移出成独立标签；拖到其他标签或终端旁边则重新分组。组合页左侧的分屏图标用于拖动整组。
